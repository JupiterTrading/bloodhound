...
console.log('🚀 Starting database seeding to known_wallets table...\n');
...
const highQualityWallets = wallets.filter(w => 
  ...
  w.twitter || 
  w.name ||
  (w.metrics.pnl_30d && w.metrics.pnl_30d > 10)
);

console.log(`📋 Filtered to ${highQualityWallets.length} high-quality wallets for seeding`);

// Prepare known_wallets records
const knownWallets = highQualityWallets
  .filter(w => w.name || w.twitter) // Must have at least name or twitter
  .map(w => ({
    address: w.wallet,
    label: w.name || w.twitter || `Wallet ${w.wallet.slice(0, 8)}`,
    category: w.is_kol ? 'kol' : 'profitable_trader',
    description: w.tags.length > 0 ? w.tags.join(', ') : null,
    twitter_handle: w.twitter || null,
    telegram_handle: w.telegram || null,
    source: w.sources.join(','),
    confidence: calculateConfidence(w),
    status: 'approved'
  }));
...
for (let i = 0; i < knownWallets.length; i += BATCH_SIZE) {
  const batch = knownWallets.slice(i, i + BATCH_SIZE);
